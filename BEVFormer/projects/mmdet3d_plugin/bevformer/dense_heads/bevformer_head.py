...
class BEVFormerHead(DETRHead):
    ...
    def __init__(self,
        ...
        self.with_hqfusion = with_hqfusion
    ...
    def forward(self, mlvl_feats, img_metas, prev_bev=None, hq_bev=None, only_bev=False):
        """Forward function.
        Args:
        ...
        if self.with_hqfusion:
            # 融合上一帧Decoder输出
            if hq_bev != None:
                qbs, qc, qw, qh = hq_bev.shape()
                hq_bev = hq_bev.squeeze().view(qc, qw*qh).permute(1, 0)
                bev_queries = bev_queries + hq_bev
